...
export function solveDesertGrid(diggingData, seasonName, strategy = 'infogain') {
  ...
  for (const piece of targetPiecesFound) {
    const validMatches = [];
    for (const pKey of targetPatternKeys) {
  ...
  const pieces = targetPiecesFound.map(p => p.key);
  ...
  if (targetPiecesFound.length > 0 && validMatchings.length > 0) {
    ...
    for (const matching of validMatchings) {
      ...
      const remainingPatternKeys = targetPatternKeys.filter(k => !matchedPatterns.includes(k));
      ...
      enumerateMatchingWorlds(0, 0, [], new Set(), new Set());
  ...
  } else if (targetPiecesFound.length === 0 && targetPatternKeys.length > 0) {
    // 0 target pieces found: enumerate target patterns
    const sortedTargetKeys = targetPatternKeys.slice().sort((a, b) => 
    ...
    searchJointWorlds(0, new Set(), new Set(), []);
  ...
  for (const [tKey] of dugItemMap.entries()) {
  ...
  // COMPLETION CHECK: All 3 targets found
  const allTargetsFound = (targetPiecesFound.length >= 3) ||
    (targetPatternKeys.length > 0 && targetPiecesFound.length >= targetPatternKeys.length);
  ...
  if (allTargetsFound) {
    ...
    for (const key of candidateKeysToScore) {
      if (dugCoords.has(key) || sandBlockedSet.has(key) || knownArtifactBodyKeys.has(key) || predictedCrabSet.has(key) || secondaryTreasureKeys.has(key)) {
        continue;
      }
      if (numWorlds > 0) {
    ...
    for (const key of filteredCandidateKeys) {
      ...
      if (isGuaranteed) {
        ...
        if (exploreWeight > 0 && validClues === 0) {
          ...
          if (isLatticePoint) score += 1000 * exploreWeight;
    ...
    for (const [key, sets] of Object.entries(artifactMap)) {
      if (dugCoords.has(key) || sandBlockedSet.has(key) || knownArtifactBodyKeys.has(key) || predictedCrabSet.has(key)) continue;
      const { x, y } = keyToCoord(key);
