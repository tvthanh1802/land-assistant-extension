  ...
  for (let i = 0; i < NUM_GAMES; i++) {
    ...
    while(true) {
      ...
      if (res.isAllCompleted) {
        ...
        if (shovels < minShovels) minShovels = shovels;
        break;
      }
      
      const next = res.bestNextDig;
      if (!next) { console.log("ERROR: NO SUGGESTION!"); break; }
      const item = game.secretBoard[next.key] || "Sand";
      digs.push({ x: next.x, y: next.y, items: { [item]: 1 } });
      shovels++;
    }
  }
  console.log(`Avg: ${(totalShovels/NUM_GAMES).toFixed(2)}, Min: ${minShovels}, Max: ${maxShovels}`);
}
runSimulations();
